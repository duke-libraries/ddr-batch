  ...
  module BatchesHelper
    ...
    def batch_action(batch)
      ...
      when Ddr::Batch::Batch::STATUS_READY
        ...
        # cf. issue #760
        #   link_to(I18n.t('ddr.batch.web.action_names.validate'), validate_batch_path(batch))
        link_to(I18n.t('ddr.batch.web.action_names.procezz'), procezz_batch_path(batch))
      when Ddr::Batch::Batch::STATUS_VALIDATED
        link_to(I18n.t('ddr.batch.web.action_names.procezz'), procezz_batch_path(batch))
      when Ddr::Batch::Batch::STATUS_RESTARTABLE
        link_to(I18n.t('ddr.batch.web.action_names.restart'), procezz_batch_path(batch))
      when Ddr::Batch::Batch::STATUS_INVALID
        link_to(I18n.t('ddr.batch.web.action_names.retry'), procezz_batch_path(batch))
      else
        "--"
      end
    end
    ...
    def show_batch_tabs
        ...
        @show_tabs = []
        @show_tabs << {
          :label => I18n.t('ddr.batch.web.tab_names.batch_info'),
          :partial => 'show_batch_info',
          :id => 'tab-default',
          :active => true
        }
        @show_tabs << { :label => I18n.t('ddr.batch.web.tab_names.batch_objects'), :partial => 'show_batch_objects', :id => 'tab-batch-objects' }
        @show_tabs
    end
      ...
      def render_batch_delete_link(batch)
        ...
        when nil, Ddr::Batch::Batch::STATUS_READY, Ddr::Batch::Batch::STATUS_VALIDATED, Ddr::Batch::Batch::STATUS_INVALID
          link_to content_tag(:span, "", :class => "glyphicon glyphicon-trash"), {:action => 'destroy', :id => batch}, :method => 'delete', :id => "batch_delete_#{batch.id}", :data => { :confirm => "#{t('ddr.batch.web.batch_deletion_confirmation', batch_id: batch.id)}" }
        end
      end
  end
end