...
describe "ddr/batch/batch_objects/show.html.erb", type: :feature, batch: true do
  ...
  context "batch object" do
    ...
    before do
      ...
      login_as batch.user
      visit(ddr_batch.batch_object_path(batch_object))
    end

    it "should display information about the batch object" do
      expect(page).to have_text("Batch Object #{batch_object.id}")
      expect(page).to have_link("#{batch.id} (#{batch.name} - #{batch.description})", :href => ddr_batch.batch_path(batch))
